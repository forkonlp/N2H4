...
setUrlByQuery <- function(query = "", targetDate = today() - 1) {
    ...
    urls <- data.frame(query_name = query, query_for_url = query_for_url, query_url = paste0(base_url, query_for_url, "&startDate=", targetDate, 
        "&endDate=", targetDate), stringsAsFactors = F)
    return(urls)
