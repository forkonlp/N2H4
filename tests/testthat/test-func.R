...
vcr::use_cassette("Comment", {
  test_that("getComment", {
    url  <-
      "https://news.naver.com/main/read.nhn?mode=LSD&mid=shm&sid1=102&oid=001&aid=0009205077"
...
vcr::use_cassette("AllComment", {
  test_that("getAllComment", {
    url  <-
      "https://news.naver.com/main/read.nhn?mode=LS2D&mid=shm&sid1=100&sid2=264&oid=015&aid=0002303155"
    test <- getAllComment(url)
    expect_equal(test$contents, c("test", "test2", "test"))
  })
})

vcr::use_cassette("Content", {
...
vcr::use_cassette("Sportsnews", {
  test_that("passSportsnews", {
    url  <-
      "https://sports.news.naver.com/wfootball/news/read.nhn?oid=477&aid=0000073768"
    test <- getContent(url)
    expect_equal(test$body, "page is not news section.")
  })
})

vcr::use_cassette("Main", {
...
vcr::use_cassette("PageNum", {
  test_that("getMaxPageNum", {
    url  <-
      "https://news.naver.com/main/list.nhn?sid2=254&sid1=102&mid=shm&mode=LS2D&date=20170427"
    test <- getMaxPageNum(url)
    expect_equal(test, 1)
  })
})

vcr::use_cassette("urllist", {
